...
The Firmware tab fetches YAML templates from the [microWakeWords](https://github.com/TaterTotterson/microWakeWords) repo on GitHub at runtime (no local fallback). Firmware build state lives under `.cache/firmware_flasher/`. Flash sessions are tracked in `FIRMWARE_SESSIONS` dict by UUID. OTA default port: `3232`. mDNS discovery runs for `ESPHOME_DISCOVERY_SECONDS` (default 2.5s).
